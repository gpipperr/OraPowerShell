...
SELECT replace(ora_database_name||'_'||SYS_CONTEXT('USERENV','HOST')||'_'||to_char(sysdate,'dd_mm_yyyy_hh24_mi')||'_backup_statistics..sql','\','_') 
...
select 
   sys_context('USERENV','SID') as my_sid,
...
begin
	select count(*) into v_count from dba_tables where owner='&&STAT_OWN.' and table_name like '&&STAT_TAB.';
	if v_count < 1 then
		dbms_stats.create_stat_table (ownname => '&&STAT_OWN.'
                            , stattab => '&&STAT_TAB.'
									 , tblspace => '&&STAT_TBLSPACE.'); 
	else
...
begin
exec dbms_stats.EXPORT_FIXED_OBJECTS_STATS ( stattab  => '&&STAT_TAB.'
...
begin
dbms_stats.EXPORT_SYSTEM_STATS (stattab  => '&&STAT_TAB.'
                              , statid   => 'SYSTEM_STATS_&&STATID_SUFFIX'
										, statown  => '&&STAT_OWN.');
end;
/
...
create table &&STAT_OWN..&&SYSSTAT_TAB. as select * from sys.aux_stats$;
...
create table &&STAT_OWN..&&IOSTAT_TAB. as select * from sys.RESOURCE_IO_CALIBRATE$;
