...
select 
        to_char(a.days,'dd.mm.yyyy') as days	
      ...
      --, round(r.size_mb * (a.writes / (t.writes_total / 1000)), 2) mb_per_user_write			
	   , round(((a.writes+a.changes) * 100) / (t.writes_total+t.changes_total ), 2) write_change_percent
      , round(r.size_mb * ((((a.writes+a.changes) * 100) / (t.writes_total+t.changes_total ))/100), 2) mb_per_user			
      , a.owner   	
 from (select trunc(completion_time) days
              ,round(sum(blocks * block_size) / 1024 / 1024, 3) size_mb
              ,DEST_ID
          from v$archived_log
