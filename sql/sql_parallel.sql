...
select  sw.sid                                               as rcvsid
		...
		, bitand(sw.p1, 16711680) - 65535 as sndrinst
		, decode(bitand(sw.p1, 65535) ,65535 ,ps.qcsid ,(select sid from gv$px_process where server_name = 'P' || to_char(bitand(sw.p1, 65535), 'fm000')and inst_id = bitand(sw.p1, 16711680) - 65535)) as sndrsid
		, decode(sw.state, 'WAITING', 'WAIT', 'NOT WAIT') as state
...
select decode(px.qcinst_id, null, s.username, ' - ' || lower(substr(pp.server_name, length(pp.server_name) - 4, 4))) "Username"
		...
		, s.sofar
		, s.totalwork
		, s.totalwork-s.sofar as openwork
		, s.units
		, s.start_time
		, s.timestamp
		, decode(px.qcinst_id, null, to_char(s.sid), px.qcsid) "QC SID"
