...
SELECT replace(ora_database_name||'_'||SYS_CONTEXT('USERENV','HOST')||'_'||to_char(sysdate,'dd_mm_yyyy_hh24_mi')||'_table_rebuild_&&TABLESPACE_NAME..sql','\','_') 
...
select 'alter table '||owner||'.'||table_name||' enable rowmovement;' from dba_tables where tablespace_name=upper('&&TABLESPACE_NAME.');

prompt Shrink all not partitioned tables
-- Alter Table <tablename> shrink space cascade;
select 'alter table '||owner||'.'||table_name||' shrink space cascade;' from dba_tables where tablespace_name=upper('&&TABLESPACE_NAME.') and PARTITIONED='NO';
...
select 'alter table '||owner||'.'||table_name||' modify partition '||p.PARTITION_NAME||' shrink space;'  
 ...
 where t.tablespace_name=upper('&&TABLESPACE_NAME.') and t.PARTITIONED='YES'
...
select 'alter table '||owner||'.'||table_name||' move;' 
  from dba_tables where tablespace_name=upper('&&TABLESPACE_NAME.') and PARTITIONED='NO';
