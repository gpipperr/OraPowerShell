...
select  inst_id
      , SESSION_ID
		, ROLE,SID
		, spid
		, WORK_MICROSEC/1000000,OVERHEAD_MICROSEC/1000000 
		,LATCHWAIT
		,LATCHSPIN
...
select a.inst_id
  ...
  from gv$sesstat a
   ...
   where value > 0 
    ...
    and sid = (select sid from v$logmnr_process where role ='preparer')
...
from gv$session_event a 
   , gv$session b
 where a.sid = b.sid
  and a.sid = (select sid from v$logmnr_process where role ='preparer')
  and a.inst_id=b.inst_id
...
--QMON Slaves Processes Consuming High Amount of CPU after Upgrade to 11.2.0.4 (Doc ID 1615165.1)
 ...
 select * 
    from gv$qmon_tasks 
 order by inst_id, task_type, task_name, task_number
/ 
