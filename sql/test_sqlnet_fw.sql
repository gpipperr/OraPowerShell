...
SELECT replace(ora_database_name||'_'||SYS_CONTEXT('USERENV','HOST')||'_'||to_char(sysdate,'dd_mm_yyyy_hh24_mi')||'_fwchek_.sql','\','_') 
...
begin
 DBMS_LOCK.sleep(60*:INTERVALS);
 dbms_output.put_line('== Info Wait time :: '||to_char(:INTERVALS)||' Wake up at :: '||to_char(sysdate,'dd.mm.yyyy hh24:mi'));
 :WAITTIME:=:WAITTIME+:INTERVALS;
end;
/ 
...
begin
 DBMS_LOCK.sleep(60*:WAITTIME);
 dbms_output.put_line('== Info Wait time :: '||to_char(:WAITTIME)||' Wake up at :: '||to_char(sysdate,'dd.mm.yyyy hh24:mi'));
 :WAITTIME:=:WAITTIME+:INTERVALS;
end;
/ 
prompt == Finish Waiting , start next try
