...
SELECT ora_database_name||'_'||SYS_CONTEXT('USERENV','HOST')||'_'||to_char(sysdate,'dd_mm_yyyy_hh24_mi')||'_remove_old_streams_logs'
  AS SPOOL_NAME_COL
FROM dual
/
...
DECLARE
 ...
 select *
   from (select lr.name
               ,lr.first_time
               ,rank() over(order by lr.first_time asc) as rang
           from DBA_REGISTERED_ARCHIVED_LOG LR
               ,DBA_LOGMNR_PURGED_LOG       LP
          where lr.first_scn < v_last_appl_scn
            and lr.name = lp.file_name
            and lr.first_time < sysdate - 1)
...
begin
  ...
  loop
		dbms_output.put_line(rpad('Info -- still need this files fro the capture ',46,' ')||' :: '||rpad(rec.name,56,' ') ||' from :: '||rec.start_time);
  end loop;
  dbms_output.put_line('Info --	');   
  dbms_output.put_line('Info -- START -> This archivelogs could be removed::');
  for rec in c_log_remove(v_last_appl_scn => v_start_Scn)
  loop
		dbms_output.put_line(rpad('Info -- try to delete this file ',46,' ')||' :: '||rpad(rec.name,56,' ') || ' from :: '||to_char(rec.first_time,'dd.mm hh24:mi'));
		begin
		...
		exception
			when others 
			then
			dbms_output.put_line(rpad('Info --try to delete this file ',46,' ')||' :: '||rpad(rec.name,56,' ') || ' Error ::'||SQLERRM);
		end;	 	  
  end loop;
