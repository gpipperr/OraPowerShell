...
select  input_type
      ...
      ,case
         when ((input_type in ('ARCHIVELOG')) and (trunc(sysdate) - trunc(start_time) > 0)) then
          'ERROR'
         else
          case
            when (trunc(sysdate) - trunc(start_time) between 7 and 14) then
             case
               when (input_type like ('CONTROLFILE%')) then
                'OK'
               else
                'ERROR'
             end
            when (trunc(sysdate) - trunc(start_time) > 14) then
             case
               when (input_type like ('CONTROLFILE%')) then
                'OK'
               else
             ...
             case
               when (status in ('COMPLETED')) then
                'OK'
               when (status in ('FAILED')) then
                'ERROR'
               else
                'WARNING'
             end
